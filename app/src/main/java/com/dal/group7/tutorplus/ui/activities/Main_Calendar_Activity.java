...
public class Main_Calendar_Activity extends AppCompatActivity {
    ...
    List<String> list;
    String text;
    DBhelper dbHelper = new DBhelper(Main_Calendar_Activity.this);
    Resources res;
                ...
                if(dateString.isEmpty() || dateString.length()==0 || dateString.equals("") || dateString == null){
                   // if (nameString.isEmpty() || nameString.length()==0 || nameString.equals("") || nameString == null) {
                        if(descString.isEmpty() || descString.length()==0 || descString.equals("") || descString == null) {
                            AlertDialog.Builder builder = new AlertDialog.Builder(Main_Calendar_Activity.this);
                  ...
                  //  }
                   ...
                   // sampleInsertions();
                    ...
                    Toast.makeText(Main_Calendar_Activity.this, "Available Date Added",Toast.LENGTH_SHORT).show();
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        switch(item.getItemId()){
            case R.id.add:
                Intent i = new Intent(Main_Calendar_Activity.this,DisplayEventActivity.class);
                startActivity(i);
        }
    ...
    public void compareDate() throws ParseException {
      ...
      //  DateFormat date = new SimpleDateFormat("MM/dd/yyyy")
        for(int i =0; i< bookinglist.size();i++)
        {
            ...
            Date date= df.parse(bookinglist.get(i).getBookingDate());
            ColorDrawable back = new ColorDrawable(getResources().getColor(R.color.colorAccent));
            caldroidFragment.setBackgroundDrawableForDate(back,date);
        }

       /* List<TimingAvailablity> calendar = new CalendarController(this).readEvent();
        if(calendar.size() > 0){

            for(TimingAvailablity obj : calendar){
    ...
    public void onSubmitClick(View v) {
        switch (v.getId()) {
            case R.id.submit_tutor:
                ...
                bookingModelArrayList=dbHelper.fetchAvailablity(dbHelper.getWritableDatabase(),tutorId);
                for(int i=0;i<bookingModelArrayList.size();i++){
                    ava = ava + " " + bookingModelArrayList.get(i).getBookingDate();
                    mesa = mesa + " " + bookingModelArrayList.get(i).getBookingDesc();
                    Toast.makeText(this,mesa,Toast.LENGTH_SHORT).show();
